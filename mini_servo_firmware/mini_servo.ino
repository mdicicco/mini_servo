/***************************************************
  Arduino firmware for the mini servo arm (Adafruit
  PCA9685 PWM driver). Paired with mini_servo_hardware.

  Serial protocol (500000 baud, 8N1):
    Host -> Arduino: six comma-separated angles in degrees,
      newline-terminated, e.g. "0.0000, 0.0000, -70.0000, -15.0000, 45.0000, -5.0000\n"
    Arduino -> Host: the same six setpoints echoed after a
      complete command. About once a second it also prints the
      six PCA9685 PWM ticks (all values typically > 90).

  The loop reads one serial byte per 10 ms tick, so hosts must
  not stream commands faster than the sketch can consume them.

  Echoed angles are commanded setpoints, not encoder feedback.
 ****************************************************/
